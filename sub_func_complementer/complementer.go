...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementInvoiceDocument(input, l)
	if err != nil {
...
func (c *SubFuncComplementer) ComplementInvoiceDocument(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		`SELECT NumberRangeID, ServiceLabel, FieldNameWithNumberRange, LatestNumber
			...
			WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'InvoiceDocument') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.InvoiceDocument = nr.LatestNumber
	return &nr, nil
}
